...
int LoadMenu(SDL_Renderer* renderer, SDL_Window* window, int w, int h, char name[], char ip[], LoadMedia media, Fonts fonts, Game_State current, UDP_Client_Config setup, Game_Route *aGameRoute, Audio settings, int* coins, bool* skinChoice[])
{
    ...
    while (running)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            else if (event.type == SDL_MOUSEBUTTONDOWN)
            {
                ...
                {
                    store(renderer, media, fonts, &coins, skinChoice);
                }
                //Store
